            ...
            <div class="card-body">
                ...
                <form method="POST" action="{{ route(Auth::getDefaultDriver() . '.exam.question.store') }}">
                                ...
                                <div class="row align-items-end">
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select name="questions[{{ $index }}][type]"
                                            ...
                                            <option value="mcq" {{ $type === 'mcq' ? 'selected' : '' }}>اختيار من متعدد
                                            </option>
                                            <option value="true_false" {{ $type === 'true_false' ? 'selected' : '' }}>صح /
                                                خطأ</option>
                                        </select>
                                    </div>
                                    <div class="col-md-4 mb-3">
                                    ...
                                    <div class="col-md-4 mb-3 answer-container">
                                        ...
                                        @else
                                            ...
                                            <select name="questions[{{ $index }}][answer]" class="form-control">
                                                <option value="true" {{ $answer === 'true' ? 'selected' : '' }}>صح
                                                </option>
                                                <option value="false" {{ $answer === 'false' ? 'selected' : '' }}>خطأ
                                                </option>
                                            </select>
                                        @endif
                                    </div>
                                        ...
                                        @foreach (['A', 'B', 'C', 'D'] as $key)
                                            <div class="col-md-3 mb-2">
                                                <input type="text"
                                                    ...
                                                    class="form-control" placeholder="خيار {{ $key }}"
                                                    value="{{ $options[$key] ?? '' }}">
                                            </div>
                                        @endforeach
                                    </div>
                    ...
                    <div class="d-flex justify-content-between">
                        <button type="button" id="add-question" class="btn btn-outline-secondary fw-bold">+ إضافة سؤال
                            آخر</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('add-question').addEventListener('click', function() {
                ...
                newRow.classList.add('question-row', 'border', 'rounded', 'p-3', 'mb-3', 'shadow-sm',
                    'bg-light');
        ...
        `;
            ...
            function handleTypeChange() {
                ...
                if (this.value === 'mcq') {
                    ...
                    ['A', 'B', 'C', 'D'].forEach(key => {
                        optionsContainer.innerHTML +=
                            `<div class="col-md-3 mb-2"><input type="text" class="form-control" name="${this.name.replace('[type]','[options]['+key+']')}" placeholder="خيار ${key}"></div>`;
                    });
                    answerContainer.innerHTML =
                        `<label class="form-label">الإجابة الصحيحة</label><input type="text" name="${this.name.replace('[type]','[answer]')}" class="form-control">`;
                } else {
                    optionsContainer.innerHTML = '';
                    answerContainer.innerHTML =
                        `<label class="form-label">صح / خطأ</label><select name="${this.name.replace('[type]','[answer]')}" class="form-control"><option value="true">صح</option><option value="false">خطأ</option></select>`;
                }
            }
